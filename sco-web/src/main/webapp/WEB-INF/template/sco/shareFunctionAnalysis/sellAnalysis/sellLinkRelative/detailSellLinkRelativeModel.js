var detailSellFn = {
	// 关闭tan页面
	close : function() {
		parent.pubTab.closeTab("明细类环比结果");
	},
	// 填写保存文件名称
	saveFile : function() {
	...
	submitSaveFileDlg : function() {
		...
		url = "detailLinkRelative_saveDetailSellToHtml_2.html?merchandiseAndSupplierCodes=" + $("input#merchandiseAndSupplierCodes").val() + 
	...
	exportDetailSellExcel : function() {
		...
		var url = "detailLinkRelative_exportDetailSellToExcel_6.html?merchandiseAndSupplierCodes=" + $("input#merchandiseAndSupplierCodes").val() + 
					"&merchandiseCodes=" + $("input#merchandiseCodes").val() + 
					"&startDate=" + $("input#startDate").val()+ 
					"&endDate=" + $("input#endDate").val() + 
					"&directJoin=" + $("input#directJoin").val()+
