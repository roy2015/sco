...
<head>  
	...
	<script type="text/javascript" >
		<#include "sco/shareFunctionAnalysis/sellAnalysis/sellLinkRelative/marketSellLinkRelativeModel.js" />
    </script>
</head>
<body>
<div >
	...
	<input type="hidden" name="supplierCodes" id="supplierCodes" value="${supplierCodes}" />
	...
	<table class="table table-condensed" style="width:1060px;margin-bottom:5px">
		<a onclick="marketSellFn.close()" plain="true" class="easyui-linkbutton" data-options="iconCls:'close'">关闭</a> 
		<a onclick="marketSellFn.saveFile();" plain="true"class="easyui-linkbutton" data-options="iconCls:'save'"> 保存 </a> 
		<a onclick="marketSellFn.exportMarketSellExcel()" plain="true" class="easyui-linkbutton" data-options="iconCls:'excel'">导出Execl</a>
	</table>
</div>
<label>整体环比信息：<br/>
...
<div >
	<table  id="marketSellLinkRelative_Grid"
		    ...
		    checkOnSelect="false"
	   		selectOnCheck="false"
			pagination = "false"
			url="marketLinkRelative_listMarketLinkRelativeGrid_2.html?merchandiseCodes=${merchandiseCodes}&startDate=${startDate?string('yyyy-MM-dd')}
				 &endDate=${endDate?string('yyyy-MM-dd')}&directJoin=${directJoin}&linkRelativeCycle=${linkRelativeCycle}&cycleCount=${cycleCount}
				 &sellRegion=${sellRegion}&rationed=${rationed}"
...
<div class="easyui-dialog" id="saveFileDlg" style="width:320px;height:160px" 
		data-options="
			...
			closable:false,closed:true,modal:true,buttons:[
					{id:'save',text:'确定',iconCls:'save',handler:function(){marketSellFn.submitSaveFileDlg()}},
					{text:'关闭',iconCls:'close',handler:function(){marketSellFn.closeSaveFileDlg()}}]">
		<br/><br/>
