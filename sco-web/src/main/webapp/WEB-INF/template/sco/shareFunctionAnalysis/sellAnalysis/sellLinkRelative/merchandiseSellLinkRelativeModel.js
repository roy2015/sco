var productSellFn = {
	// 关闭tan页面
	close : function() {
		parent.pubTab.closeTab("单品环比结果");
	},
	// 填写保存文件名称
	saveFile : function() {
		var rows = $('#productSell_Grid').datagrid("getRows");
		if (rows.length==0) {
	...
	submitSaveFileDlg : function() {
		...
		url = "merchandiseLinkRelative_saveProductSellToHtml_2.html?merchandiseAndSupplierCodes=" + $("input#merchandiseAndSupplierCodes").val() +
				...
				"&linkRelativeCycle=" + $("input#linkRelativeCycle").val()+
				"&cycleCount=" + $("input#cycleCount").val();
	...
	//导出销售同比
	exportProductSellExcel : function() {
		var rows = $('#productSell_Grid').datagrid("getRows");
		if (rows.length==0) {
			$.messager.alert("提示", "数据为空不能导出!");
			return;
		}
		var url = "merchandiseLinkRelative_exportProductSellToExcel_6.html?merchandiseAndSupplierCodes=" + $("input#merchandiseAndSupplierCodes").val() +
					"&merchandiseCodes=" + $("input#merchandiseCodes").val() + 
					"&startDate=" + $("input#startDate").val()+ 
					"&endDate=" + $("input#endDate").val() + 
					"&directJoin=" + $("input#directJoin").val()+
