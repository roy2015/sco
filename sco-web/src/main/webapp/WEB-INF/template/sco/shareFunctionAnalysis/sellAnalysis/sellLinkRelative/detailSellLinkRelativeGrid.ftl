...
<head>  
	...
	<script type="text/javascript" >
		<#include "sco/shareFunctionAnalysis/sellAnalysis/sellLinkRelative/detailSellLinkRelativeModel.js" />
    </script>
</head>
<body>
<div >
	...
	<table class="table table-condensed" style="width:1060px;margin-bottom:5px">
		<a onclick="detailSellFn.close()" plain="true" class="easyui-linkbutton" data-options="iconCls:'close'">关闭</a> 
		<a onclick="detailSellFn.saveFile();" plain="true"class="easyui-linkbutton" data-options="iconCls:'save'"> 保存 </a> 
		<a onclick="detailSellFn.exportDetailSellExcel()" plain="true" class="easyui-linkbutton" data-options="iconCls:'excel'">导出Execl</a>
	</table>
</div>
	<table  id="detailSell_Grid"
			...
			url="detailLinkRelative_listDetailTypeInfo_2.html?merchandiseAndSupplierCodes=${merchandiseAndSupplierCodes}&rationed=${rationed}"
		    data-options="rownumbers:true"> 
		<thead>
			<tr>
				<th data-options="field:'detailTypeName',width:100,sortable:false,align:'center'">
					<span>明细类名称</span>
				</th>
				<th data-options="field:'detailTypeSku',width:100,sortable:false,align:'center'">
					<span>SKU数</span>
				</th>
			</tr>
		</thead>
...
<div >
	<table  id="detailSellLinkRelative_Grid"
			class="easyui-datagrid"
		    title="明细类环比信息"
		    singleSelect="true"
		    checkOnSelect="false"
	   		selectOnCheck="false"
			pagination = "false"
			url="detailLinkRelative_listDetailLinkRelativeGrid_2.html?merchandiseCodes=${merchandiseCodes}&startDate=${startDate?string('yyyy-MM-dd')}
...
<div class="easyui-dialog" id="saveFileDlg" style="width:320px;height:160px" 
		data-options="
			...
			closable:false,closed:true,modal:true,buttons:[
					{id:'save',text:'确定',iconCls:'save',handler:function(){detailSellFn.submitSaveFileDlg()}},
					{text:'关闭',iconCls:'close',handler:function(){detailSellFn.closeSaveFileDlg()}}]">
		<br/><br/>
