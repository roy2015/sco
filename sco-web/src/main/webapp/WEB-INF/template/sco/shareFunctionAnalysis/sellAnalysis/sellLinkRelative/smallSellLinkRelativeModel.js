var smallSellFn = {
	// 关闭tan页面
	close : function() {
		parent.pubTab.closeTab("小分类环比结果");
	},
	// 填写保存文件名称
	saveFile : function() {
		var rows = $('#smallSell_Grid').datagrid("getRows");
		if (rows.length==0) {
	...
	submitSaveFileDlg : function() {
		...
		url = "smallLinkRelative_saveSmallSellToHtml_2.html?merchandiseAndSupplierCodes=" + $("input#merchandiseAndSupplierCodes").val()  + 
	...
	//导出销售同比
	exportSmallSellExcel : function() {
		var rows = $('#smallSell_Grid').datagrid("getRows");
		if (rows.length==0) {
			$.messager.alert("提示", "数据为空不能导出!");
			return;
		}
		var url = "smallLinkRelative_exportSmallSellToExcel_6.html?merchandiseAndSupplierCodes=" + $("input#merchandiseAndSupplierCodes").val() + 
					"&merchandiseCodes=" + $("input#merchandiseCodes").val() + 
					"&startDate=" + $("input#startDate").val()+ 
					"&endDate=" + $("input#endDate").val() + 
					"&directJoin=" + $("input#directJoin").val()+
