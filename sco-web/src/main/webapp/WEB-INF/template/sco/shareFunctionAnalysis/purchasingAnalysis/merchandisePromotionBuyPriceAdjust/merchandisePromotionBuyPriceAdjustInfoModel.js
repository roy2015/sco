var merchandisePromotionBuyPriceAdjustInfoFn = {
	// 关闭tan页面
	close : function() {
		parent.pubTab.closeTab("促销进价调整明细报表");
	},
	// 填写保存文件名称
	saveFile : function() {
		var row = $('#erchandisePromotionBuyPriceAdjustInfo_grid').datagrid("getRows");
		if (row == ""||row==null) {
	...
	submitSaveFileDlg : function() {
		...
		var url = "merchandisePromotionBuyPriceAdjust_saveSearchDataForm_2.html?";
	...
	execlMerchandise : function() {
		...
		var url = "merchandisePromotionBuyPriceAdjust_exporterchandisePromotionBuyPriceAdjustInfoToExcel_6.html?";
		var prarm = "minDate=" + $("#minDate").val() + "&maxDate=" + $("#maxDate").val() + "&show=" + $("#show").val() + "&warehouseCode=" + $("#warehouseCode").val() 
			+"&merchandiseAndSupplierCodes=" + $("#merchandiseAndSupplierCodes").val();
		url += prarm;
		window.location = url;
		$.messager.show({
