var merchandiseLongBuyPriceAdjustInfoFn = {
	// 关闭tan页面
	close : function() {
		parent.pubTab.closeTab("长期进价调整明细报表-商品");
	},
	// 填写保存文件名称
	saveFile : function() {
		var row = $('#merchandiseLongBuyPriceAdjustInfo_grid').datagrid("getRows");
		if (row == ""||row==null) {
	...
	submitSaveFileDlg : function() {
		...
		var url = "merchandiseLongBuyPriceAdjust_saveSearchDataForm_2.html?";
	...
	execlMerchandise : function() {
		...
		var url = "merchandiseLongBuyPriceAdjust_exportMerchandiseLongBuyPriceAdjustInfoToExcel_6.html?";
		var prarm = "minDate=" + $("#minDate").val() + 
					"&maxDate=" + $("#maxDate").val() + 
					"&show=" + $("#show").val() + 
					"&regionCode=" + $("#sellRegion").val() + 
