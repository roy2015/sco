var accessoryStockInfoFn = {
	// 关闭tan页面
	close : function() {
		parent.pubTab.closeTab("进货明细报表-辅料");
	},
	// 填写保存文件名称
	saveFile : function() {
		var row = $('#accessoryStockInfo_grid').datagrid("getRows");
		if (row == ""||row==null) {
	...
	submitSaveFileDlg : function() {
		...
		var url = "accessoryStockDetail_saveSearchDataForm_2.html?";
	...
	execlMerchandise : function() {
		...
		var url = "accessoryStockDetail_exportAccessoryStockInfoToExcel_6.html?";
		var prarm = "minDate=" + $("#minDate").val() + 
					"&maxDate=" + $("#maxDate").val() + 
					"&show=" + $("#show").val() + 
					"&regionCode=" + $("#sellRegion").val() + 
