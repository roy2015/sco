var smCatPrice1 = null;
var smCatPrice2 = null;
...
$(document).ready(
		function() {
			...
			smCatPrice1 = utils.grid($('#smCatPrice1'));
			smCatPrice1.registerExtFilters("centreTypeCode", "smallTypeCode", "regionCode", "dxRoleCode", "dlRoleCode", "startYear1", 
					"startMonth1", "endYear1", "endMonth1", "startYear2", "startMonth2", "endYear2", "endMonth2", 
					"merchandiseName", "merchandiseCode");
			smCatPrice1.initFilters({
...
//						firstIn = false;
					...
					/*if (length == 4 && $.param(obj).indexOf("order") > -1) {
						var param = smCatPrice1.getFilterValue();
				...
				onLoadSuccess : function(data) {
					$('#smCatPrice1').datagrid("clearSelections");
					smCatPriceFn.allShowChart(data.rows, "1");
				}
			});
			
			smCatPrice2 = utils.grid($('#smCatPrice2'));
			smCatPrice2.registerExtFilters("centreTypeCode", "smallTypeCode", "regionCode", "dxRoleCode", "dlRoleCode", "startYear1", 
					"startMonth1", "endYear1", "endMonth1", "startYear2", "startMonth2", "endYear2", "endMonth2",
					"merchandiseName", "merchandiseCode");
			smCatPrice2.initFilters({
				onBeforeLoad : function(obj) {
					// 清空查询
					if (clearSearch) {
						return true;
					...
					/*if (length == 4 && $.param(obj).indexOf("order") > -1) {
						var param = smCatPrice2.getFilterValue();
						if ($.param(param).length > 0) {
							return true;
						}
						window.parent.$.messager.alert("提示", "请输入至少一项查询条件");
				...
				onLoadSuccess : function(data) {
					$('#smCatPrice2').datagrid("clearSelections");
					smCatPriceFn.allShowChart(data.rows, "2");
				}
			});
	}
);
...
var smCatPriceFn = {
	...
	formatBigPercent:function(val) {
		...
		return smCatPriceFn.formatPercent(val);
	},
	...
	clonePriceRegion : function(btn){
		...
		var data = '<tr name="priceRegion'+ length+'">'
			...
			+ '价格大于等于：&nbsp;'
			+ '<input class="easyui-numberbox" style="width:80px;" id=minPrice'+ length +' name=minPrice'+ length +' maxlength="12" onchange="smCatPriceFn.floorPrice('+"'"+length+"'"+')" onblur="smCatPriceFn.floorPriceBlur('+"'"+length+"'"+')"/>'
			+ '&nbsp;元&nbsp;&nbsp;价格小于：&nbsp;'
			+ '<input class="easyui-numberbox" style="width:80px;" id=maxPrice'+ length +' name=maxPrice'+ length +' maxlength="12" onchange="smCatPriceFn.limtPrice('+"'"+length+"'"+')" onblur="smCatPriceFn.limtPriceBlur('+"'"+length+"'"+')" />'
			+ '&nbsp;元&nbsp;&nbsp;价格步长：&nbsp;'
			+ '<input class="easyui-numberbox" style="width:80px;" id=addPrice'+ length +' name=addPrice'+ length +' maxlength="12" onchange="smCatPriceFn.addPrice('+"'"+length+"'"+')" onblur="smCatPriceFn.addPriceBlur('+"'"+length+"'"+')" />'
			+ '&nbsp;元&nbsp;&nbsp;&nbsp;<input type="button" value="+" onclick="smCatPriceFn.clonePriceRegion(this)" '
			+ 'style="width: 28px;" />'
			+ '&nbsp;&nbsp;&nbsp;<input type="button" value="-" onclick="smCatPriceFn.removeCurrentRow(this)" '
			+ 'style="width: 28px;" />';
			+ '</td></tr>'
	...
	searchAllCategory:function() {
		smCatPriceFn.displayChart();//隐藏图表
		$("#con1").hide();
		$("#con2").hide();
		
		$('#smCatPrice1').datagrid('loadData', {total:0, rows:[]});
		$('#smCatPrice2').datagrid('loadData', {total:0, rows:[]});
		
		smCatPriceFn.showOrHiddenCol('showColumn', 'hideColumn');
		smCatPriceFn.searchData('A', smCatPrice1);
	},
	...
	searchDetCategory:function() {
		...
		smCatPriceFn.showOrHiddenCol('hideColumn', 'showColumn');
		smCatPriceFn.searchData('D', smCatPrice1);
	},
	
	//隐藏或显示列
	showOrHiddenCol:function(flag1, flag2) {
		for (var i in smCol) {//所有列
			$("#smCatPrice1").datagrid(flag1, smCol[i]);
		}
		
		for (var i in smCol) {//所有列
			$("#smCatPrice2").datagrid(flag1, smCol[i]);
		}

		//------------------------------------------------
		
		for (var i in smDetCol) {//明细列
			$("#smCatPrice1").datagrid(flag2, smDetCol[i]);
		}
		for (var i in smDetCol) {//明细列
			$("#smCatPrice2").datagrid(flag2, smDetCol[i]);
		}
		...
		$('#smCatPrice1').datagrid('getColumnOption','soldUnits').title = noFlag;
		$('#smCatPrice2').datagrid('getColumnOption','soldUnits').title = noFlag;
//		$('#allCatPrice1').datagrid();
	...
	searchData:function(showPanel, dataGrid) {
		...
		var msg = smCatPriceFn.validateSearchCondition();
		if(msg.length > 1) {
			window.parent.$.messager.alert('提示', msg);
			return; 
		}
		var param1 = smCatPriceFn.generatePara('1', dataGrid.getFilterValue());
		if ('D' == showPanel) {//明细统计
			...
			$("#smBar1").html("");
			$("#smCatPrice1").datagrid("loadData", []);
			loading('smCatPrice1');
			$.ajax({
				type : "POST",
				url : 'smallTypePrice_listDetailData_2.html',
				data : param1,
				dataType : 'JSON',
				success : function(data) {
					smCatPriceFn.pageData(1,data.rows);
				}
			});
			
			/*$('#smCatPrice1').datagrid({			//grid查询
				url : 'smallTypePrice_listDetailData_2.html',
				queryParams : param1
			});*/
		} else {//所有价格带统计
			...
			smCatPriceFn.calculateTotal(1, param1);//上方统计
			$('#smCatPrice1').datagrid({			//grid查询
				pagination : false,
				data : [],
				url : 'smallTypePrice_listCollectData_2.html',
				queryParams : param1
			});
		...
		if($('#startYear2').val()) {//加载第二个datagrid
			var param2 = smCatPriceFn.generatePara('2', dataGrid.getFilterValue());
			//日期区间1
			...
			if ('D' == showPanel) {//明细数据
				...
				$.ajax({
					...
					dataType : 'JSON',
					success : function(data) {
						smCatPriceFn.pageData(2,data.rows);
					}
				});
				/*$('#smCatPrice2').datagrid({
					url : 'smallTypePrice_listDetailData_2.html',
					queryParams : param2
				});*/
			} else {			//所有数据
				...
				smCatPriceFn.calculateTotal(2, param2);
				$('#smCatPrice2').datagrid({
					pagination : false,
					data : [],
					url : 'smallTypePrice_listCollectData_2.html',
	...
	exportToExcel:function() {
		var data1 = $('#smCatPrice1').datagrid('getRows');
		var data2 = $('#smCatPrice2').datagrid('getRows');
		//是否查出数据
		...
		var param2 = smCatPriceFn.getSaveAndExportPara();
		var url = "smallTypePrice_exportAllCategoryToExcel_6.html?";
		if ($('#showPanel').val() == 'D') {
			param2.merchandiseCode = $('#merchandiseCode').val();
			param2.merchandiseName = $('#merchandiseName').val();
			...
			url = "smallTypePrice_exportDetCategoryToExcel_6.html?";
		} else {
			param2.merchandiseCode = null;
			param2.merchandiseName = null;
		}
	...
	calculateTotal : function(i, param) {
		$.ajax({
			type : "POST",
			url : "smallTypePrice_listCalculateTotal_2.html",
			data : param,
			dataType : 'JSON',
			success : function(data) {
				if (data.success) {//查询成功
					...
					if (value.indexOf("|") >= 0) {//有数据
						var arr = value.split("|");
						smCatPriceFn.setCalculateTotal(i, arr[0], arr[1], arr[2], arr[3]);
					} else {//无数据
						smCatPriceFn.setCalculateTotal(i, "", "", "", "");
					}
				} else {//查询出错
	...
	saveFile:function() {
		...
		$("#smSaveFileDlg").window('open');//打开窗口
	},
	...
	submitSaveFileDlg:function() {
		...
		var param2 = smCatPriceFn.getSaveAndExportPara();
		var url = "smallTypePrice_saveAllCategory_2.html?";
		if ($('#showPanel').val() == 'D') {//保存明细数据
			...
			url = "smallTypePrice_saveDetCategory_2.html?";
		} else {
			param2.merchandiseCode = null;
			param2.merchandiseName = null;
		}
		...
		$.post(url + $.param(param2), {
			...
			if (json.success) {// 成功
				...
				$("#smSaveFileDlg").window('close');// 关闭窗口
				$.messager.show({
					title : '提示',
					msg : msg
				});
	...
	getSaveAndExportPara:function() {
		var param2 = smCatPriceFn.generatePara('2', smCatPrice1.getFilterValue());
		//日期区间1
		param2.firSYear1 = $('#startYear1').val();
		param2.firSMonth1 = $('#startMonth1').val();
		...
		param2.merTitle = $('#smallTypeCode').combobox('getText');
		param2.module = "SM";//模块名称
		return param2;
	},
	...
	clearSearch:function() {
		clearSearch = true;
		$('#smCatPrice_search').form('reset');
		clearComboboxOptions();
		smCatPriceFn.displayPriceRegion(false);//隐藏价格带
		smCatPriceFn.displayDmc(false);//隐藏明细报表中只显示指定名称/编号的商品
		smCatPriceFn.displayChart();//隐藏图表
		myChart = new Array();
		$("#con1").hide();
		$("#con2").hide();
	...
	selectStartYear:function(obj, tarId, i) {
		...
		smCatPriceFn.selectStartMon('startYear' + i,'startMonth' + i,'endYear' + i,'endMonth' + i);
	},
	...
	selectEndYear:function(obj, tarId, i) {
		...
		smCatPriceFn.selectEndMon('startYear'+i,'startMonth'+i,'endYear'+i,'endMonth'+i);
	},
	...
	floorPriceBlur:function(i) {
		if (!$('#minPrice' + i).val()) {
			...
			smCatPriceFn.vlidPrice(i, '#maxPrice' + i, '#addPrice' + i)
		}
	},
	...
	addPriceBlur:function(i) {
		...
		smCatPriceFn.vlidPrice(i, '#UNKNOW', '#addPrice' + i)
	},
	...
	validateSearchCondition:function() {
		...
		if(!smallTypeCode) {
			...
			$("span[name=smName]").html($('#smallTypeCode').combobox('getText'));
		}
		...
		if ($('#handSetPri').attr('checked')=='checked') {
			...
			$("tr[name*=priceRegion").each(function(i, obj){
				...
				smCatPriceFn.floorPriceBlur(name.substring(name.length - 1));
				var min = $("#minPrice" +i).val();
				var max = $("#maxPrice" +i).val();
				var add = $("#addPrice" +i).val();
				if (!min || !max || !add) {
	...
	pageData:function(index,data) {
		clearSearch = true;
		$("#smCatPrice" + index).datagrid({
			pagination : true,
			url:'',
			data : data.slice(0, 30)
		});
		var pager = $("#smCatPrice" + index).datagrid("getPager");
		pager.pagination({
			total : data.length,
			onSelectPage : function(pageNo, pageSize) {
				var start = (pageNo - 1) * pageSize;
				var end = start + pageSize;
				$("#smCatPrice" + index).datagrid("loadData",
