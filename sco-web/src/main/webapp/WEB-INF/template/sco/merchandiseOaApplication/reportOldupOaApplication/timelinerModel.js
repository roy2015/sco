...
var timelinerFn = {
	...
	click3:function(){
		 ...
		 $.post('reports_listMyReportTypeForMOA_5.html', null, function(data){
			 $(data).each(function(i, obj){
				 $("#oldrptTypeCode").append("<option value="+ obj.id +">"+obj.text+"</option>");
			 });
		 },"json");
		 utils.grid($('#oldOaReportAnalyGrid')).initFilters({noParamCanSort:true});//启用查询条件
	},
	click4:function(){
		 $('#intention-1').panel("close");
		 $('#intention-2').panel("close");
	...
	click5:function(){
		 ...
		 utils.grid($('#appScheOldGrid')).initFilters({
			 ...
			 onClickRow : function(rowIndex, rowData){
				...
				if (editIndex != rowIndex){
					 if (appScheOldFn.endEditing()){
						 editIndex = rowIndex;
						 alert(editIndex);
						 $('#appScheOldGrid').datagrid('selectRow', rowIndex)
						 .datagrid('beginEdit', rowIndex);
					 } else {
						 $('#appScheOldGrid').datagrid('selectRow', editIndex);
					 }
				}
			 }*/
		 });//启用查询条件
			 ...
			 applicationCode : '${applicationCode}'
				...
				if (data.success) {
					 window.parent.pubTab.showTab('关联商品原料与公示网站原料', 'powertree', 
						 ...
						 +"?intentionAndSupplierCodes=${intentionAndSupplierCodes}&oaType=OLD");
				} else {
