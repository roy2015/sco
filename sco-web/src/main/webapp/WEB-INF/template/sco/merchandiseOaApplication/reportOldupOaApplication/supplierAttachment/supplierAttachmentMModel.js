			...
			$("#uploadMDForm").form(
					"submit",
					{
						url : "supplierAttachmentM_completeImportsupplierAttachmentM_2.html?merchandiseCode=" + merchandiseCode + "&supplierCode=" + supplierCode + "&applicationCode="
								+ $("input#applicationCode").val() + "&intentionAndSupplierCodes=" + $("input#intentionAndSupplierCodes").val()+"&applicationType=mo",
...
var supplierAttachmentMGLFn = {
	...
	downloadFileGL : function(value, row, index) {
		...
		return '<a href=javascript:void(0) onClick=supplierAttachmentMGLFn.downloadOriginalFileGL("' + encodeURIComponent(row.path) + '","' + row.accountingCode + '","'+type+'")>' + name + '</a><br>';
	},
