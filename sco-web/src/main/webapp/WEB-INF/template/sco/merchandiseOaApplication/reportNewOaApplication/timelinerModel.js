...
var timelinerFn = {
	...
	click2:function(){
		 ...
		 $('#intention-6').panel("close");
		 $('#intention-7').panel("close");
		 $('#supplierCertificateM_grid').datagrid('reload');
		 $('#supplierCertificateM_grid_gl').datagrid('reload');
	},
	click3:function(){
		 $('#intention-1').panel("close");
		 $('#intention-2').panel("close");
		 ...
		 $('#intention-6').panel("close");
		 $('#intention-7').panel("close");
		 $('#merchandisePrice_Grid0').datagrid('reload');
		 $('#merchandiseSame_Grid0').datagrid('reload');
		 $('#merchandiseSell_Grid0').datagrid('reload');
	...
	click4:function(){
		 ...
		 $.post('reports_listMyReportTypeForMOA_5.html', null, function(data){
			 $(data).each(function(i, obj){
				 $("#newrptTypeCode").append("<option value="+ obj.id +">"+obj.text+"</option>");
			 });
		 },"json");
		 utils.grid($('#newOaReportAnalyGrid')).initFilters({noParamCanSort:true});//启用查询条件
	},
	click5:function(){
		 $('#intention-1').panel("close");
		 $('#intention-2').panel("close");
	...
	/*click6:function(){
		 ...
		 $('#intention-7').hide();
		 $('#intention-8').hide();
		 utils.grid($('#appScheNewGrid')).initFilters({
			...
			onDblClickRow : function(rowIndex, rowData){
				...
				if (editIndex != rowIndex){
					 if (appScheNewFn.endEditing()){
						 editIndex = rowIndex;
						 $('#appScheNewGrid').datagrid('selectRow', rowIndex)
						 .datagrid('beginEdit', rowIndex);
					 } else {
						 $('#appScheNewGrid').datagrid('selectRow', editIndex);
					 }
				}
			}
		 });//启用查询条件
...
//								+"data=%3Cdata%20xmlns%3D%22http%3A%2F%2Fschemas.cordys.com%2Ftask%2F1.0%2F%22%2F%3E&organization=o"
//								+"%3Dlaiyifen%2Ccn%3Dcordys%2Ccn%3DdefaultInst1%2Co%3Dlaiyifen";
							window.open(url);
						} else {
...
//			}else{
//				$.messager.alert('提示', '没有OA系统同步数据，不能查看OA审批意见!');
//			}
//		},"json");
	},
	click7:function(){
		 ...
		 $('#intention-5').panel("close");
		 $('#intention-6').panel("close");
		 $('#intention-7').panel("open");
		 $.post('wlInfoNew_ifApproved_2.html', {
			 applicationCode : '${applicationCode}'
		 }, function(data){ 
				if (data.success) {
					 $.post('wlInfoOld_listWlInfoByApplicationCode_2.html', {
						 applicationCode : '${applicationCode}',
						 ...
						 if (rs.success) {
							 window.parent.pubTab.showTab('关联商品原料与公示网站原料', 'powertree', 
									 ...
									 +"?intentionAndSupplierCodes=" + rs.rows + "&oaType=NEW");
						 } else {
							 $.messager.alert("提示", "请先去物料信息模块填写新的SAP物料号");
						 }
					}, "json");
				} else {
