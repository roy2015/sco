...
var timelinerFn = {
	...
	click4:function(){
		 ...
		 utils.grid($('#compareOaReportAnalyGrid')).initFilters({
			 noParamCanSort:true,
//			 onBeforeLoad : function() {
//				campOaReportAnalyFn.setDatagridHeight();
//			 },
...
//			 },
		 ...
		 $.ajax({
				...
				success : function(data){
					if(data.success) {
						...
						window.open("http://eip.laiyifen.com/oa/lyf/XunJia.nsf/0/"+codes[0]+"?opendocument");
					} else {
	...
	click7:function(){
		 ...
		 $('#intention-7').show();
		 $('#intention-8').hide();
		 utils.grid($('#appScheCompareGrid')).initFilters({
			 noParamCanSort:true,
			 onLoadSuccess:function(data){
				 editIndex = undefined;
				 $('#appScheCompareGrid').datagrid('clearSelections');
				 for (var i in data.rows) {
					 $('#appScheCompareGrid').datagrid('beginEdit', i);
					 $('#aogDate' + i).datebox({
						 editable:false,
			...
			onDblClickRow : function(rowIndex, rowData){
				...
				if (editIndex != rowIndex){
					 if (appScheCompareFn.endEditing()){
						 editIndex = rowIndex;
						 $('#appScheCompareGrid').datagrid('selectRow', rowIndex)
						 .datagrid('beginEdit', rowIndex);
					 } else {
						 $('#appScheCompareGrid').datagrid('selectRow', editIndex);
					 }
				}
			}*/
		 });//启用查询条件
