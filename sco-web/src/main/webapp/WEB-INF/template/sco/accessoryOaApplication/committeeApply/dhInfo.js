...
var dhInfoFn = {
		...
		showUpdateDhFile : function(applicationCode) {
			...
			var title = '上传新文件';
			var href = "committeeApply_showInsertDhFile_1.html?intentionCode=" + intentionCode + "&supplierCode=" + supplierCode + "&applicationCode=" + applicationCode;
			...
			var dlg = utils.showDlg({
				...
				buttons : [ {
					...
					handler : function() {
						dhInfoFn.submitInsertDhfile(dlg);
					},
					iconCls : 'save'
				}, {
		...
		submitInsertDhfile : function(dlg) {
			var uploadVar = $('#committeeUpload').val();
			if (uploadVar == '') {
			...
			utils.form("dhinfo_form").submit("committeeApply_insertDhInfo_2.html", null, function() {
				dlg.dialog('close');
		...
		deleteDhInfo : function(intentionCode) {
			...
			if (record.length == 0) {
				...
				utils.confirm("操作确认", "确认删除大货信息吗?", function() {
					utils.post("committeeApply_deleteDhInfo_2.html?dhId=" + dhId, null, function() {
						$('#dhInfo_grid').datagrid('reload');
					});
				});
			}
		...
		downloadDhFile : function(path) {
			window.location ="committeeApply_downloadDhFile_6.html?path="+ encodeURIComponent(path);
		},
