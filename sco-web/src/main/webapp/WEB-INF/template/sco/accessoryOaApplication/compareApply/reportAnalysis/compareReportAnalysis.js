...
var campOaReportAnalyFn = {
	...
	linkReport:function() {
		...
		campOaReportAnalyFn.seaFir = true;
		var title = '关联报表';
		var href = 'analysisReportCompare_showLinkAnalysisReportCompareForm_1.html';
		
		var dlg = utils.showDlg({
			title:title,href:href,width:540,height:430,
			buttons:[{text:'确认关联',id:'sbt',handler:function(){campOaReportAnalyFn.submitLinkForm(dlg);},iconCls:'save'},
			        {text:'关闭页面',handler:function(){dlg.dialog('close');},iconCls:'close'}
			],onLoad:function() {
				$("#typeCode").val(val);//报表类型编号
				$("#typeName").html(text);//报表类型名称
				campOaReportAnalyFn.addRptGrid = utils.grid($('#comOalinkReportGrid'));
				campOaReportAnalyFn.addRptGrid.registerExtFilters("typeCode", "reportsName", "minSaveDate", "maxSaveDate");
				campOaReportAnalyFn.addRptGrid.initFilters({
					onBeforeLoad:function(obj) {
						if (campOaReportAnalyFn.seaFir) {
							campOaReportAnalyFn.seaFir = false;
							return false;
						}
						...
						$('input[id=selectedRt]').val(campOaReportAnalyFn.added);//保存已选中的商品
					},
					...
					onClickRow:campOaReportAnalyFn.selectedMd,
					onCheck:campOaReportAnalyFn.selectedMd,
					onUncheck:campOaReportAnalyFn.selectedMd,
					onCheckAll:campOaReportAnalyFn.selectedMd,
					onUncheckAll:campOaReportAnalyFn.selectedMd
				});
			}
		});
	...
	searchRpt:function() {
		campOaReportAnalyFn.seaFir = false;
		var param = campOaReportAnalyFn.addRptGrid.getFilterValue();
		$('#comOalinkReportGrid').datagrid('load', param);
	},
	
	//打开成本分析文件
	formatFileName:function(value, rowData) {
		return '<a href=javascript:void(0) onclick=campOaReportAnalyFn.downloadOriginalFile("' + encodeURIComponent(rowData.reportsUrl) + '","'+ encodeURIComponent(rowData.reportsName) + '")>' + value + '</a><br>';
	},
	...
	submitLinkForm:function(dlg) {
		...
		utils.post("analysisReportCompare_insertLinkAnalysisReportCompare_2.html",
			{
		    ...
		    reportsCode : rows.reportsCode
				...
				dlg.dialog('close');
				campOaReportAnalyFn.reloadSystemOaReportAnalyGrid();
		});
	...
	confirmRemoveLinkSystem:function() {
		var rows = $("#compareOaReportAnalyGrid").datagrid("getSelections");
		if (rows.length < 1) {
	...
	removeLink:function() {
		...
		campOaReportAnalyFn.closeDlg();
		utils.post('analysisReportCompare_deleteAnalysisReportSystem_2.html', 
			{	
			 applicationCode : '${applicationCodeNow}', 
			 quotedCodes : '${quotedCodes}',
	...
	confirmRemoveLinkUpload:function() {
		...
		parent.window.utils.confirm('提示','是否确认删除此分析报表',function(){
			utils.post("analysisReportCompare_deleteAnalysisReportUpload_2.html",
					{
					applicationCode : '${applicationCodeNow}', 
					quotedCodes : '${quotedCodes}',
				 	reportsCodes : reportsCode
				 	},function(){
						campOaReportAnalyFn.reloadUploadOaReportAnalyGrid();
			});
		});
	},
	...
	confirmRemoveLinkPurOrder:function() {
		...
		reportsCode = reportsCode.substring(0, reportsCode.length - 1);
		parent.window.utils.confirm('提示','是否确认删除此分析报表',function(){
			utils.post("analysisReportCompare_deleteAnalysisReportPurOrder_2.html",
				{
				applicationCode : '${applicationCodeNow}', 
				quotedCodes : '${quotedCodes}',
				reportsCodes : reportsCode
				},function(){
					campOaReportAnalyFn.reloadPurOrderOaReportAnalyGrid();
				}
			);
		});
	},
	...
	rptUpload:function() {
		...
		$("#rptUpload_form").form("submit", {
			url : "analysisReportCompare_uploadAnalysisReport_2.html",
	...
	showPurOrderUploadForm:function(isEdit){
		var title='上传申购单';
		var href='analysisReportCompare_showPurOrderUploadForm_1.html';
		var dlg=utils.showDlg({
			title:title,href:href,width:400,height:259,
			buttons:[{text:'确定',
						handler:function(){campOaReportAnalyFn.purOrderUpload(dlg);},iconCls:'save'},
			        {text:'关闭',
	...
	purOrderUpload:function(dlg) {
		...
		$("#nonFood_form").form("submit", {
			url : "analysisReportCompare_uploadAnalysisReportPurOrder_2.html",
	...
	reloadSystemOaReportAnalyGrid:function() {
		$('#compareOaReportAnalyGrid').datagrid("clearSelections");
		$('#compareOaReportAnalyGrid').datagrid('load');
	},
