<#compress>
	<script type="text/javascript" >
		<#include "sco/accessoryOaApplication/nonFoodApply/applicationschedule/applicationScheduleNonFood.js" />
    </script>
    
	<div id="appScheNonFoods_toolbar">
		<a onclick="appScheNonFoodFn.save();" plain="true" class="easyui-linkbutton" data-options="iconCls:'save'">
			保存
		</a>
		<a onclick="appScheNonFoodFn.close();" plain="true" class="easyui-linkbutton" data-options="iconCls:'close'">
			关闭
		</a>
	</div>
	<div>
		<table id="appScheNonFoodGrid" 
			pagination = "true"
			pagePosition = "bottom"
			pageSize = "20"
			pageList = "[ 10, 20, 30, 40 ]"
			toolbar="#appScheNonFoods_toolbar"
			style="height:350px"
			data-options="rownumbers:true,
			url : 'applicationScheduleNonFood_listApplicationScheduleNonFood_2.html?applicationCode=${applicationCodeNow}&quotedCodes=${quotedCodes}'
		">  
			<thead>
				<tr>
					...
					<th rowspan="2" data-options="field:'normalFlowDate',width:200,align:'right',
						formatter:function(value, rowData, rowIndex){
							var val = '';
							if (value != null) val = value;
							return '<input id=normalFlowDate'+ rowIndex+' onblur=appScheNonFoodFn.calculDifferenceDate(\''+rowIndex+'\') value=' + val +' >';
						}
					">
						正常作业流程时间（天）
					</th>
