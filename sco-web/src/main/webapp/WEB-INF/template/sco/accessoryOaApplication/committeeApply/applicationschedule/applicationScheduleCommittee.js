...
var appScheCommitteeFn = {
	
	//保存
	save:function(){
		var rows = $("#appScheCommittGrid").datagrid("getSelections");
		if(rows.length < 1) {
		...
		$(rows).each(function(i, obj){
			i = $('#appScheCommittGrid').datagrid('getRowIndex', obj);//当前数据所在行
			...
			$('#appScheCommittGrid').datagrid('endEdit', i);//结束在编辑的行
			// 获取编辑
			msg += appScheCommitteeFn.validateData(obj, i) + "\r\n";
			obj.applicationCode='${applicationCodeNow}';
		});
		if (!flag) {
			appScheCommitteeFn.beginEdit();
			window.parent.$.messager.alert('提示', '正常作业流程时间必须是数字');
			return;
		}
//		$('#appScheCommittGrid').datagrid('acceptChanges');
		//是否通过验证
		...
		var url='applicationScheduleCommittee_insertApplicationScheduleCommittee_2.html';
		$.post(url,
				{
				...
				quotedCodes : '${quotedCodes}'
					...
					if (data.success) {
						...
						$('#appScheCommittGrid').datagrid('load');
	...
	//开启编辑
	beginEdit:function() {
		var rows = $("#appScheCommittGrid").datagrid("getRows");
		$(rows).each(function(i, obj){
			$('#appScheCommittGrid').datagrid('beginEdit', i);//开启编辑
			$('#aogDate' + i).datebox({
				editable:false,
	...
	//结束编辑状态
	endEditing:function(){
		 //$('#appScheCommittGrid').datagrid('endEdit', editIndex);//结束在编辑的行
		 $('#appScheCommittGrid').datagrid('unselectRow', editIndex);//结束在编辑的行
		return true;
		/* 校验改行
		 * if (editIndex == undefined){return true;}
		 alert(editIndex);
		 if ($('#appScheCommittGrid').datagrid('validateRow', editIndex)){
			 var ed = $('#appScheCommittGrid').datagrid('getEditor', {index:editIndex,field:'factoryNo'});
			 var factoryName = $(ed.target).combobox('getText');
			 $('#appScheCommittGrid').datagrid('getRows')[editIndex]['factoryName'] = factoryName;
			 $('#appScheCommittGrid').datagrid('endEdit', editIndex);
			 editIndex = undefined;
			 return true;
		 } else {
			 return false;
	...
	calculRealityFlowDate:function() {
		...
		appScheCommitteeFn.calculDifferenceDate(index);
	},
