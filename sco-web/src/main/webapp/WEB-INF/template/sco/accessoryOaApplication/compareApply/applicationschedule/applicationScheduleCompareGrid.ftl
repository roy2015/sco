<#compress>
	<script type="text/javascript" >
		<#include "sco/accessoryOaApplication/compareApply/applicationschedule/applicationScheduleCompare.js" />
    </script>
    
	<div id="ScheCompare_toolbar">
		<a onclick="appScheCompareApplyFn.save();" plain="true" class="easyui-linkbutton" data-options="iconCls:'save'">
			保存
		</a>
		<a onclick="appScheCompareApplyFn.close();" plain="true" class="easyui-linkbutton" data-options="iconCls:'close'">
			关闭
		</a>
	</div>
	<div>
		<table id="appScheCompareGrid" 
			pagination = "true"
			pagePosition = "bottom"
			pageSize = "20"
			pageList = "[ 10, 20, 30, 40 ]"
			toolbar="#ScheCompare_toolbar"
			style="height:350px"
			data-options="rownumbers:true,
			url : 'applicationScheduleCompare_listApplicationScheduleCompare_2.html?applicationCode=${applicationCodeNow}&quotedCodes=${quotedCodes}'
		">  
			<thead>
				<tr>
					...
					<th rowspan="2" data-options="field:'normalFlowDate',width:200,align:'right',
						formatter:function(value, rowData, rowIndex){
							var val = '';
							if (value != null) val = value;
							return '<input id=normalFlowDate'+ rowIndex+' onblur=appScheCompareApplyFn.calculDifferenceDate(\''+rowIndex+'\') value=' + val +' >';
						}
					">
						正常作业流程时间（天）
					</th>
				...
				closable:false,closed:true,modal:true,buttons:[
						{
							...
							handler:function(){
								appScheCompareApplyFn.closeErrDlg();
							}
						}
				]
		">
			<textArea id="cpErrMsg" readonly="true" style="margin: 0px; width: 315px; height: 167px;border:0;resize: none;"></textArea>
		</div>
	</div>
</#compress>