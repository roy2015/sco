...
var timelinerFn = {
	...
	click2:function(){
		 ...
		 $('#intention-8').hide();
		/* alert();
		 utils.grid($('#supplierCertificateA_grid')).initFilters({});
		 utils.grid($('#supplierCertificateA_grid_gl')).initFilters({});*/
	...
	click4:function(){
		 ...
		 utils.grid($('#nonFoodOaReportAnalyGrid')).initFilters({
			 noParamCanSort:true,
//			 onBeforeLoad:function() {
//					noneOaReportAnalyFn.setDatagridHeight();
//			 },
...
//			},
		 ...
		 $.ajax({
				...
				success : function(data){
					if(data.success) {
						...
						window.open("http://eip.laiyifen.com/oa/lyf/fspgysyj.nsf/0/"+codes[0]+"?opendocument");
					} else {
	...
	click7:function(){
		 ...
		 $('#intention-7').show();
		 $('#intention-8').hide();
		 utils.grid($('#appScheNonFoodGrid')).initFilters({
			 noParamCanSort:true,
			 onLoadSuccess:function(data){
				 editIndex = undefined;
				 $('#appScheNonFoodGrid').datagrid('clearSelections');
				 for (var i in data.rows) {
					 $('#appScheNonFoodGrid').datagrid('beginEdit', i);
					 $('#aogDate' + i).datebox({
						 editable:false,
			...
			onDblClickRow : function(rowIndex, rowData){
				...
				if (editIndex != rowIndex){
					 if (appScheNonFoodFn.endEditing()){
						 editIndex = rowIndex;
						 $('#appScheNonFoodGrid').datagrid('selectRow', rowIndex)
						 .datagrid('beginEdit', rowIndex);
					 } else {
						 $('#appScheNonFoodGrid').datagrid('selectRow', editIndex);
					 }
				}
			}*/
		 });//启用查询条件
