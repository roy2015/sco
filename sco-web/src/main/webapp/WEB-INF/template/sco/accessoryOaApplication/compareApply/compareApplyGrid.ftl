...
<head>  
    ...
    <script type="text/javascript" >
		<#include "sco/accessoryOaApplication/compareApply/compareApplyModel.js" />
		<#include "sco/common/masterDataType.js" />
    </script>
</head>
<body>
	<!-- 辅料意向品管理主表 --> 
	<div id="compareApply_toolbar">
	 <form id="compareApply_search">
	...
	<table class="table table-condensed" style="width:1060px;margin-bottom:5px">
		<a onclick="compareApplyFn.search();"plain="true" class="easyui-linkbutton" data-options="iconCls:'search'">
				搜索
		</a>
		<a onclick="compareApplyFn.clearFilter();" plain="true" class="easyui-linkbutton" data-options="iconCls:'clear'">
			清空查询
		</a>
		<a id="insert" onclick="compareApplyFn.showInsert(null);" plain="true" class="easyui-linkbutton" data-options="iconCls:'add'" style="display: <#if !action.hasFuncPower("com.powere2e.sco.accessoryOaApplication.compare")>none</#if>;">
			新增申请单
		</a>
		<a id="remove" onclick="compareApplyFn.remove();" plain="true" class="easyui-linkbutton" data-options="iconCls:'delete'" style="display:<#if !action.hasFuncPower("com.powere2e.sco.accessoryOaApplication.compare")>none</#if>;">
			删除申请单
		</a>
		<a id="close" onclick="compareApplyFn.close();" plain="true" class="easyui-linkbutton" data-options="iconCls:'close'" style="display:<#if !action.hasFuncPower("com.powere2e.sco.accessoryOaApplication.compare")>none</#if>;">
			关闭申请单
		</a>
		<a onclick="compareApplyFn.allowOaSynchronous();" plain="true" class="easyui-linkbutton" data-options="iconCls:'active'">
			允许OA同步
		</a>
		<a onclick="compareApplyFn.undoOaSynchronous();" plain="true" class="easyui-linkbutton" data-options="iconCls:'cancel'">
			撤销OA同步
		</a>
		<a id="lookApproveOpinion" onclick="compareApplyFn.lookApproveOpinion();" plain="true" class="easyui-linkbutton" data-options="iconCls:'edit'" style="display: <#if !action.hasFuncPower("com.powere2e.sco.accessoryOaApplication.compare")>none</#if>;">
			查看OA审批意见
		</a>
	</table>	
</div>

<table  id="compareApplyGrid"
		    fit="true"
		    iconCls= "icon-save"
		    singleSelect="true"
		    checkOnSelect="false"
			...
			toolbar="#compareApply_toolbar"
			url='compareApply_listCompareApplyIntentionApplication_2.html'
        ...
        <thead>
        	<tr>
				...
				<th data-options="field:'applicationCode',width:120,sortable:true,formatter : function(value, rowData, rowIndex) {
				    if(value!=null){
					return '<a href=# style=color:maroon;cursor:hand; onclick=compareApplyFn.showInsert(\''+rowData.applicationCode+'\') >'+value+'</a>'
					}else{
					return value;
					}
					}">
