var supplierAttachmentAFn = {
	...
	searchAQS : function() {
		...
		//var href = 'supplierAttachmentA_showaccessoryQuotedScanGrid_1.html?enquiryCode=' + enquiryCode+"&supplierCode="+supplierCode;
	...
	importFromExcel : function() {
		var rows = $('#supplierAttachmentA_grid').datagrid("getSelections");
		if (rows.length != 1) {
			window.parent.$.messager.alert('提示', '请选择一条报价记录');
			return;
		}
		...
		if (type != "") {
			...
			$("#uploadMDForm").form(
					"submit",
					{
						url : "supplierAttachmentA_completeImportsupplierAttachmentA_2.html?" + "intentionCode=" + record.intentionCode + "&intentionSupplierCode=" + record.intentionSupplierCode
								+ "&enquiryCode=" + record.enquiryCode + "&quotedCodeScan=" + $("input#quotedCodes").val() + "&quotedCode=" + record.quotedCode + "&applicationCode="
								+ $("input#applicationCodeNow").val() + "&supplierCode=" + record.intentionSupplierCode + "&type=ac",
