var compareApplyGridUtil=null;
var firstIn = true;// 是否第一次进入页面
var clearSearch = false;// 清空查询
$(document).ready(function(){
	compareApplyGridUtil=utils.grid($('#compareApplyGrid'));
	compareApplyGridUtil.registerExtFilters("intentionSupplierCode", "intentionSupplierName", "intentionCode", "intentionName", 
	...
	compareApplyGridUtil.initFilters({
		onBeforeLoad:function(obj) {
			...
			if(length == 5 && $.param(obj).indexOf("order") > -1) {
				var param = compareApplyGridUtil.getFilterValue();
		...
		onLoadSuccess:function(){
			...
			$('#compareApplyGrid').datagrid("clearSelections");
		}
	});
});
var compareApplyFn = {
		...
		showInsert : function(applicationCode) {
			...
			var compareApplyGrid = $('#compareApplyGrid').datagrid('getChecked');
			if (compareApplyGrid.length == 0) {
				$.messager.alert('提示', '<center>请至少选择一条记录！</center>');
				return;
			} else {
				var applicationFlag=true;
				$.each(compareApplyGrid, function(index, item) {
			...
			var tabName = '新增询价比较OA申请';
			}else{
				var tabName ='询价比较申请单-'+applicationCode;
			}
				var url = "compareApply_addCompareApply_1.html?quotedCodes="+quotedCodes+"&applicationCode="+applicationCode;
		...
		//	accessoryIntentionGridUtil.clearFilter();
			...
			remove:function(){
				var applicationCode=[];
				var applicationFlag=true;
				var applyGrid = $('#compareApplyGrid').datagrid('getChecked');
				if (applyGrid.length == 0) {
					...
					if(applicationCode.length>0){
						utils.confirm("操作确认","确认删除OA申请吗?",function(){
							utils.post("compareApply_deleteApplication_2.html?applicationCode="+applicationCode,null,function(){
								//intentionUtil.refresh();
								$('#compareApplyGrid').datagrid('reload');
							});
						});
					}else{
				...
				if (applyGrid.length == 0) {
					...
					if(applicationCode.length>0){
						utils.confirm("操作确认","确认关闭OA申请吗?",function(){
							utils.post("compareApply_closeApplication_2.html?applicationCode="+applicationCode,null,function(){
								//intentionUtil.refresh();
								$('#compareApplyGrid').datagrid('reload');
							});
						});
				...
				if (applyGrid.length == 0) {
					...
					if(applicationCode.length>0){
						utils.confirm("操作确认","确认允许OA同步吗?",function(){
							utils.post("compareApply_allowOaSynchronous_2.html?applicationCode="+applicationCode,null,function(){
								//intentionUtil.refresh();
								$('#compareApplyGrid').datagrid('reload');
							});
						});
			...
			if (applyGrid.length == 0) {
				...
				if(applicationCode.length>0){
					utils.confirm("操作确认","确认撤销OA同步吗?",function(){
						utils.post("compareApply_undoOaSynchronous_2.html?applicationCode="+applicationCode,null,function(){
							//intentionUtil.refresh();
							$('#compareApplyGrid').datagrid('reload');
						});
					});
				...
				if (applyGrid.length == 0) {
					...
					if(applicationCode==null||applicationCode==""){
						...
						if(oaApplicationCode==null||oaApplicationCode==""){
							...
							window.open("http://eip.laiyifen.com/oa/lyf/XunJia.nsf/0/"+oaApplicationCode+"?opendocument");
						}
					}
				}
			}
