...
<head>  
    ...
    <script type="text/javascript" >
		<#include "sco/accessoryOaApplication/committeeApply/committeeApplyModel.js" />
		<#include "sco/common/masterDataType.js" />
    </script>
</head>
<body>
	<!-- 辅料意向品管理主表 --> 
	<div id="committeeApply_toolbar">
	 <form id="committeeApply_search">
	...
	<table class="table table-condensed" style="width:1060px;margin-bottom:5px">
		<a onclick="committeeApplyFn.search();"plain="true" class="easyui-linkbutton" data-options="iconCls:'search'">
				搜索
		</a>
		<a onclick="committeeApplyFn.clearFilter();" plain="true" class="easyui-linkbutton" data-options="iconCls:'clear'">
			清空查询
		</a>
		<a id="insert" onclick="committeeApplyFn.showInsert(null);" plain="true" class="easyui-linkbutton" data-options="iconCls:'add'" style="display: <#if !action.hasFuncPower("com.powere2e.sco.accessoryOaApplication.committee")>none</#if>;">
			新增申请单
		</a>
		<a id="remove" onclick="committeeApplyFn.remove();" plain="true" class="easyui-linkbutton" data-options="iconCls:'delete'" style="display:<#if !action.hasFuncPower("com.powere2e.sco.accessoryOaApplication.committee")>none</#if>;">
			删除申请单
		</a>
		<a id="close" onclick="committeeApplyFn.close();" plain="true" class="easyui-linkbutton" data-options="iconCls:'close'" style="display:<#if !action.hasFuncPower("com.powere2e.sco.accessoryOaApplication.committee")>none</#if>;">
			关闭申请单
		</a>
		<a onclick="committeeApplyFn.allowOaSynchronous();" plain="true" class="easyui-linkbutton" data-options="iconCls:'active'">
			允许OA同步
		</a>
		<a onclick="committeeApplyFn.undoOaSynchronous();" plain="true" class="easyui-linkbutton" data-options="iconCls:'cancel'">
			撤销OA同步
		</a>
		<a id="lookApproveOpinion" onclick="committeeApplyFn.lookApproveOpinion();" plain="true" class="easyui-linkbutton" data-options="iconCls:'edit'" style="display: <#if !action.hasFuncPower("com.powere2e.sco.accessoryOaApplication.committee")>none</#if>;">
			查看OA审批意见
		</a>
	</table>	
</div>

<table  id="committeeApplyGrid"
		    fit="true"
		    iconCls= "icon-save"
			pagination = "true"
			pagePosition = 'bottom'
			...
			toolbar="#committeeApply_toolbar"
			url='committeeApply_listCommitteeApplyIntentionApplication_2.html'
        ...
        <thead>
        	<tr>
				...
				<th data-options="field:'applicationCode',width:120,sortable:true,formatter : function(value, rowData, rowIndex) {
				    if(value!=null){
					return '<a href=# style=color:maroon;cursor:hand; onclick=committeeApplyFn.showInsert(\''+rowData.applicationCode+'\') >'+value+'</a>'
					}else{
					return value;
					}
					}">
