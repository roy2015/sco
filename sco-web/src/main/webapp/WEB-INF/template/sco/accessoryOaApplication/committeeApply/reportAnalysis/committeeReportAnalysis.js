...
var comOaReportAnalyFn = {
	...
	linkReport:function() {
		...
		comOaReportAnalyFn.seaFir = true;
		var title = '关联报表';
		var href = 'analysisReportCommittee_showLinkAnalysisReportCommitteeForm_1.html';
		
		var dlg = utils.showDlg({
			title:title,href:href,width:540,height:430,
			buttons:[{text:'确认关联',id:'sbt',handler:function(){comOaReportAnalyFn.submitLinkForm(dlg);},iconCls:'save'},
			        {text:'关闭页面',handler:function(){dlg.dialog('close');},iconCls:'close'}
			],onLoad:function() {
				$("#typeCode").val(val);//报表类型编号
				$("#typeName").html(text);//报表类型名称
				comOaReportAnalyFn.addRptGrid = utils.grid($('#comOalinkReportGrid'));
				comOaReportAnalyFn.addRptGrid.registerExtFilters("typeCode", "reportsName", "minSaveDate", "maxSaveDate");
				comOaReportAnalyFn.addRptGrid.initFilters({
					onBeforeLoad:function(obj) {
						if (comOaReportAnalyFn.seaFir) {
							comOaReportAnalyFn.seaFir = false;
							return false;
						}
						...
						$('input[id=selectedRt]').val(comOaReportAnalyFn.added);//保存已选中的商品
					},
					...
					onClickRow:comOaReportAnalyFn.selectedMd,
					onCheck:comOaReportAnalyFn.selectedMd,
					onUncheck:comOaReportAnalyFn.selectedMd,
					onCheckAll:comOaReportAnalyFn.selectedMd,
					onUncheckAll:comOaReportAnalyFn.selectedMd
				});
			}
		});
	...
	searchRpt:function() {
		comOaReportAnalyFn.seaFir = false;
		var param = comOaReportAnalyFn.addRptGrid.getFilterValue();
		$('#comOalinkReportGrid').datagrid('load', param);
	},
	
	//打开成本分析文件
	formatFileName:function(value, rowData) {
		return '<a href=javascript:void(0) onclick=comOaReportAnalyFn.downloadOriginalFile("' + encodeURIComponent(rowData.reportsUrl) + '","'+ encodeURIComponent(rowData.reportsName) + '")>' + value + '</a><br>';
	},
	...
	submitLinkForm:function(dlg) {
		...
		utils.post("analysisReportCommittee_insertLinkAnalysisReportCommittee_2.html",
			{
		    ...
		    reportsCode : rows.reportsCode
				...
				dlg.dialog('close');
				comOaReportAnalyFn.reloadSystemOaReportAnalyGrid();
		});
	...
	confirmRemoveLinkSystem:function() {
		var rows = $("#committeeOaReportAnalyGrid").datagrid("getSelections");
		if (rows.length < 1) {
	...
	removeLink:function() {
		...
		comOaReportAnalyFn.closeDlg();
		utils.post('analysisReportCommittee_deleteAnalysisReportSystem_2.html', 
				{	
				 applicationCode : '${applicationCodeNow}', 
				 quotedCodes : '${quotedCodes}',
	...
	confirmRemoveLinkUpload:function() {
		...
		parent.window.utils.confirm('提示','是否确认删除此分析报表',function(){
			utils.post("analysisReportCommittee_deleteAnalysisReportUpload_2.html",
					{
					applicationCode : '${applicationCodeNow}', 
					quotedCodes : '${quotedCodes}',
				 	reportsCodes : reportsCode
				 	},function(){
						comOaReportAnalyFn.reloadUploadOaReportAnalyGrid();
			});
		});
	},
	...
	confirmRemoveLinkPurOrder:function() {
		...
		reportsCode = reportsCode.substring(0, reportsCode.length - 1);
		parent.window.utils.confirm('提示','是否确认删除此分析报表',function(){
			utils.post("analysisReportCommittee_deleteAnalysisReportPurOrder_2.html",
				{
				applicationCode : '${applicationCodeNow}', 
				quotedCodes : '${quotedCodes}',
				reportsCodes : reportsCode
				},function(){
					comOaReportAnalyFn.reloadPurOrderOaReportAnalyGrid();
				}
			);
		});
	},
	...
	rptUpload:function() {
		...
		$("#rptUpload_form").form("submit", {
			url : "analysisReportCommittee_uploadAnalysisReport_2.html",
	...
	showPurOrderUploadForm:function(isEdit){
		var title='上传申购单';
		var href='analysisReportCommittee_showPurOrderUploadForm_1.html';
		var dlg=utils.showDlg({
			title:title,href:href,width:400,height:259,
			buttons:[{text:'确定',
						handler:function(){comOaReportAnalyFn.purOrderUpload(dlg);},iconCls:'save'},
			        {text:'关闭',
						handler:function(){dlg.dialog('close');},iconCls:'close'}
			],onLoad:function() {
				$("#purOrder_form").form('validate');
			}
		});
		$(".window-shadow").remove();
	},
	...
	purOrderUpload:function(dlg) {
		...
		if(!$("#purOrder_form").form('validate')) {
			return;
		}
		
		$("#purOrder_form").form("submit", {
			url : "analysisReportCommittee_uploadAnalysisReportPurOrder_2.html",
	...
	reloadSystemOaReportAnalyGrid:function() {
		...
		$('#committeeOaReportAnalyGrid').datagrid('load');
	},
