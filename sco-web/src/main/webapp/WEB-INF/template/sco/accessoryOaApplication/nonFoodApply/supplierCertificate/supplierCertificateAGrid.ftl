...
<script type="text/javascript">
	<#include "sco/accessoryOaApplication/nonFoodApply/supplierCertificate/supplierCertificateAModel.js" />
</script>
<!-- 意向品和供应商列表 -->
...
<table id="supplierCertificateA_grid" class="easyui-datagrid" title="本次申请引进的商品列表" fit="true" singleSelect="false" pagination="true" pagePosition='bottom' pageSize="10" pageList="[10, 15, 20 ,50]"
	...
	<thead>
		<tr>
			...
			<th data-options="field:'intentionCode',width:40,sortable:false">意向品编号</th>
			<th data-options="field:'intentionName',sortable:false">意向品名称</th>
			<th
				data-options="field:'intentionSupplierCode',width:120,sortable:false,formatter:function(value,row){
																				 if(value==null){
			...
			<th
				data-options="field:'intentionSupplierName',width:120,sortable:false,formatter:function(value,row){
																				 if(value==null){
...
<table id="supplierCertificateA_grid_gl" class="easyui-datagrid" toolbar="#supplierCertificateA_toolbar" title="已选择/上传证件列表：" fit="true" singleSelect="false" pagination="true" pagePosition='bottom'
	pageSize="5" pageList="[ 5, 10, 15, 20 ]" fitColumns="true" url="supplierCertificateA_listSupplierCertificateA_2.html?applicationCode=${applicationCodeNow}" data-options="rownumbers:true">
	<thead>
		<tr>
			<th align="center" data-options="checkbox:true"></th>
			<th
