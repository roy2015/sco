...
import com.powere2e.sco.interfaces.service.accessoryoaapplication.CompareApplyService;
 ...
 * @since 2015年3月16日
 */
public class CompareApplyAction extends UploadUtils {
	...
	private CompareApplyService compareApplyService;
	...
	protected void beforeBuild() {
		compareApplyService = (CompareApplyService) ConfigFactory.getInstance().getBean("compareApplyService");
	...
	public void doShowCompareApplyGrid() throws Exception {
		this.forwardPage("sco/accessoryOaApplication/compareApply/compareApplyGrid.ftl");
	}
	 ...
	 */
	@Authority(privilege = "com.powere2e.sco.accessoryOaApplication")
	public void doAddCompareApply() throws Exception {
		...
		if (applicationCodeNow == null || "null".equals(applicationCodeNow)) {
			...
			for (int i = 0; i < qcs.length; i++) {
				CompareApply compareApply = new CompareApply();
				SubscribeAccessory subscribeAccessory = new SubscribeAccessory();
				compareApply = compareApplyService.loadCompareApplyIntentionApplication(qcs[i]);
				Map<String, Object> map = new HashMap<String, Object>();
				map.put("enquiryCode", compareApply.getEnquiryCode());
				...
				subscribeAccessory.setAccessoryName(compareApply.getIntentionName());
				subscribeAccessory.setIntentionCode(compareApply.getIntentionCode());
				subscribeAccessory.setEnquiryCode(compareApply.getEnquiryCode());
				subscribeAccessory.setAccessoryEnquiryQuotedCountList(accessoryEnquiryQuotedCountList);
			...
			if (subscribeAccessoryList != null && subscribeAccessoryList.size() > 0) {
				...
				for (int i = 0; i < qcs.length; i++) {
					...
					compareApply = compareApplyService.loadCompareApplyIntentionApplication(qcs[i]);
					Map<String, Object> map1 = new HashMap<String, Object>();
					map1.put("enquiryCode", compareApply.getEnquiryCode());
					List<AccessoryEnquiryQuotedCount> accessoryEnquiryQuotedCountList = accessoryEnquiryQuotedCountService.listAccessoryEnquiryQuotedCount(map1, null);
					subscribeAccessory.setAccessoryEnquiryQuotedCountList(accessoryEnquiryQuotedCountList);
					subscribeAccessory.setAccessoryName(compareApply.getIntentionName());
					subscribeAccessory.setIntentionCode(compareApply.getIntentionCode());
		...
		this.forwardPage("sco/accessoryOaApplication/compareApply/addCompareApply.ftl");
	}
	 ...
	 */
	@Authority(privilege = "com.powere2e.sco.accessoryOaApplication")
	public void doListCompareApplyIntentionApplication() throws Exception {
		Map<String, Object> map = getCompareApply().toMap();
		...
		List<CompareApply> list = compareApplyService.listCompareApplyIntentionApplication(map, this.getPageInfo());
	...
	public void doListWlInfo() throws Exception {
		...
		if (applicationQuoted != null) {
			...
			list = wlInfoService.listWlInfo(map, null);
				for (int i = 0; i < qcs.length; i++) {
					CompareApply compareApply = compareApplyService.loadCompareApplyIntentionApplication(qcs[i]);
					WlInfo wlInfo = new WlInfo();
					wlInfo.setIntentionCode(compareApply.getIntentionCode());
					wlInfo.setIntentionName(compareApply.getIntentionName());
					wlInfo.setSupplierCode(compareApply.getSupplierCode() == null ? compareApply.getIntentionSupplierCode() : compareApply.getSupplierCode());
					wlInfo.setSupplierName(compareApply.getSupplierName() == null ? compareApply.getIntentionSupplierName() : compareApply.getSupplierName());
					wlInfo.setQuotedCode(compareApply.getQuotedCode());
					wlInfo.setEnquiryCode(compareApply.getEnquiryCode());
					Map<String, Object> map2 = new HashMap<String, Object>();
					map2.put("enquiryCode", compareApply.getEnquiryCode());
	 ...
	 */
	@Authority(privilege = "com.powere2e.sco.accessoryOaApplication")
	public void doLoadCompareApplyIntentionApplication() throws Exception {
		CompareApply compareApply = compareApplyService.loadCompareApplyIntentionApplication(asString("quotedCode"));
		this.forwardData(true, compareApply, null);
	}
	...
	public void doInsertApplicationReportAccessoryAndSubscribeAccessory() throws Exception {
		...
		String applicationType = BusinessConstants.ApplicationType.ACCESSORY_XJBJ.toString();

		if (compareApplyService.compareInsertUpdateDeleteIsOk(quotedCodes, applicationCodeNow, applicationType)) {
			...
			try {
				...
				compareApplyService.insertApplicationReportAccessoryAndSubscribeAccessory(applicationReportAccessory, subscribeAccessoryList);
	...
	public String saveToHtml(String fileName, Map<String, Object> paraMap) {
		String tarPath;
		try {                                      
			tarPath = FreeMarkerUtil.generateHtml("compare/applicationReportAccessory.ftl",
					"compare".concat("/").concat(fileName), paraMap);
		} catch (Exception e) {
			return "转换报表模板时异常";
		}
	...
	public void doShowInsertDhFile() throws Exception {
		...
		this.forwardPage("sco/accessoryOaApplication/compareApply/dhInfoForm.ftl");
	}
	...
	public void doDeleteDhInfo() throws Exception {
		...
		if (dhId != null) {
			...
			compareApplyService.deleteDhInfo(dhinfoId);
			this.forwardData(true, null, this.getText("public.success"));
		}
	}
	...
	public void doDeleteApplication() {
		...
		if (StringUtils.isNotBlank(applicationCode)) {
			...
			compareApplyService.deleteApplication(applicationCodes);
	...
	public void doCloseApplication() {
		...
		if (StringUtils.isNotBlank(applicationCode)) {
			String applicationCodes[] = applicationCode.split(",");
			compareApplyService.closeApplication(applicationCodes);
	...
	public void doUndoOaSynchronous() {
		...
		if (StringUtils.isNotBlank(applicationCode)) {
			String applicationCodes[] = applicationCode.split(",");
			compareApplyService.undoOaSynchronous(applicationCodes);
	...
	public void doAllowOaSynchronous() {
		String check = "";
		String applicationCode = this.asString("applicationCode");
		check = compareApplyService.checkApplication(applicationCode);
		if (StringUtils.isNotBlank(applicationCode) && "".equals(check)) {
			compareApplyService.allowOaSynchronous(applicationCode);
			this.forwardData(true, null, this.getText("public.success"));
		} else {
	...
	public void doInsertWlInfo() throws Exception {
		...
		try {
			if (!StringUtils.isBlank(rows)) {
				...
				this.compareApplyService.insertWlInfo(dataArray);
			}
		} catch (Exception e) {
			this.forwardData(false, null, "保存失败，请重试");
			return;
	 ...
	 *             可能抛出Exception异常
	 */
	private CompareApply getCompareApply() throws Exception {
		CompareApply compareApply = new CompareApply();
		this.asBean(compareApply);
		return compareApply;
	}
	...
	private String getOaApplicationPrimaryKey() {
		//return "OA".concat(SequenceFactory.getInstance().nextID("OA_APPLICATION"));
