 ...
 * @version 1.0
 */
public class SmallLinkRelativeServiceImpl extends ServiceImpl implements SmallLinkRelativeService {
	...
	private SmallLinkRelativeDao smallLinkRelativeDao;
	private Font font = null;

	public SmallLinkRelativeDao getSmallLinkRelativeDao() {
		return smallLinkRelativeDao;
	}

	public void setSmallLinkRelativeDao(SmallLinkRelativeDao smallLinkRelativeDao) {
		this.smallLinkRelativeDao = smallLinkRelativeDao;
	}

	public static SmallLinkRelativeService getInstance() {
		return (SmallLinkRelativeService) ConfigFactory.getInstance().getBean("smallLinkRelativeService");
	}

	// 查询小分类信息
	public List<LinkRelative> listSmallTypeInfo(Map<String, Object> map, PageInfo pageInfo) {
		return smallLinkRelativeDao.listSmallTypeInfo(map, pageInfo);
	}

	// 查询小分类销售环比
	public List<SellLinkRelativeAnalysis> querySmallSellLinkRelative(Map<String, Object> map, PageInfo pageInfo) {
		return smallLinkRelativeDao.querySmallSellLinkRelative(map, pageInfo);
	}
	...
	public String saveSmallSellToHtml(String fileName, Map<String, Object> paraMap) throws Exception {
		String tarPath = null;
		List<LinkRelative> smallTypeList = this.listSmallTypeInfo(paraMap, null);
		List<SellLinkRelativeAnalysis> merchandiseList = this.querySmallSellLinkRelative(paraMap, null);

		paraMap.put("smallTypeList", smallTypeList);
		paraMap.put("merchandiseList", merchandiseList);

		try {
			String fileDirName = "sellAnalysis/sellLinkRelative/smallSellLinkRelativeReport.ftl";
			tarPath = FreeMarkerUtil.generateHtml(fileDirName, "smallSellLinkRelative".concat("/").concat(fileName), paraMap);
		} catch (Exception e) {
			return "转换报表模板时异常!";
		}
		...
		if (file.exists()) {
			try {
				Reports myReport = new Reports(BusinessConstants.myReportType.SSS.toString(), fileName, tarPath.replace(ConfigPath.getUploadFilePath(), ""));
				ReportsServiceImpl.getInstance().insertReports(myReport);
			} catch (Exception e) {
				file.delete();
				LoggerUtil.logger.error("SmallLinkRelativeServiceImpl.saveSmallSellToHtml删除文件["+ file.getPath() +"]");
	...
	public void exportSmallSellToExcel(Map<String, Object> map, ServletOutputStream out) throws Exception {
		List<LinkRelative> list = this.listSmallTypeInfo(map, null);
		List<SellLinkRelativeAnalysis> merchandiseList = this.querySmallSellLinkRelative(map, null);
		Workbook wb = new XSSFWorkbook();
		Sheet sheet1 = wb.createSheet();
		Sheet sheet2 = wb.createSheet();
		wb.setSheetName(0, "小分类信息");
		wb.setSheetName(1, "小分类环比信息");
		font = wb.createFont();
	...
	private void writeOneRowDate(Sheet sheet, Integer rowIndex, LinkRelative linkRelative, CellStyle strStyle, CellStyle amtStyle) {
		...
		cell.setCellValue(linkRelative.getSmallTypeName());// 明细类名称

		cell = row.createCell(j++);
		cell.setCellStyle(strStyle);
		cell.setCellValue(linkRelative.getSmallTypeSku());// SKU数
	}
