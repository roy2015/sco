...
import com.powere2e.sco.interfaces.service.sharefunctionanalysis.sellanalysis.sellyearonyearanalysis.SmallSellYearOnYearService;
 ...
 * 
 * @author Joyce.li
 * @since 2015年6月8日 下午7:17:07
 * @version 1.0
 */
public class SmallSellYearOnYearServiceImpl extends ServiceImpl implements SmallSellYearOnYearService {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private SmallSellYearOnYearDao smallSellYearOnYearDao;
	private Font font = null;

	public SmallSellYearOnYearDao getSmallSellYearOnYearDao() {
		return smallSellYearOnYearDao;
	}

	public void setSmallSellYearOnYearDao(SmallSellYearOnYearDao smallSellYearOnYearDao) {
		this.smallSellYearOnYearDao = smallSellYearOnYearDao;
	}

	public static SmallSellYearOnYearService getInstance() {
		return (SmallSellYearOnYearService) ConfigFactory.getInstance().getBean("smallSellYearOnYearService");
	}

	// 查询小分类同比信息
	@Override
	public List<SellYearOnYearAnalysis> querySmallSellYearOnYear(Map<String, Object> map, PageInfo pageInfo) {
		return smallSellYearOnYearDao.querySmallSellYearOnYear(map, pageInfo);
	}
	...
	public String saveSmallSellToHtml(String fileName, Map<String, Object> paraMap) throws Exception {
		String tarPath;
		List<SellYearOnYear> smallTypeList = this.listSmallTypeInfo(paraMap, null);
		List<SellYearOnYearAnalysis> smallSellList = this.querySmallSellYearOnYear(paraMap, null);
		List<SellYearOnYearAnalysis> smallSellSumList = this.querySmallSellSum(paraMap, null);
		if (!smallSellList.isEmpty()) {
			smallSellList.addAll(smallSellSumList);
		}
		// paraMap.clear();
		paraMap.put("smallTypeList", smallTypeList);
		paraMap.put("smallSellList", smallSellList);

		try {
			// 默认查看1年同比数据，用模版一
			String fileDirName = "sellAnalysis/sellYearOnYearAnalysis/smallSellYearOnYearReport.ftl";
			tarPath = FreeMarkerUtil.generateHtml(fileDirName, "smallSellYearOnYear".concat("/").concat(fileName), paraMap);
		} catch (Exception e) {
			return "转换报表模板时异常!";
		}
		...
		if (file.exists()) {
			try {
				Reports myReport = new Reports(BusinessConstants.myReportType.SCS.toString(), fileName, tarPath.replace(ConfigPath.getUploadFilePath(), ""));
				ReportsServiceImpl.getInstance().insertReports(myReport);
			} catch (Exception e) {
				file.delete();
				LoggerUtil.logger.error("SmallSellYearOnYearServiceImpl.saveSmallSellToHtml删除文件["+ file.getPath() +"]");
	...
	public void exportSmallSellToExcel(Map<String, Object> map, ServletOutputStream out) throws Exception {
		List<SellYearOnYear> list = this.listSmallTypeInfo(map, null);
		List<SellYearOnYearAnalysis> smallList = this.querySmallSellYearOnYear(map, null);
		List<SellYearOnYearAnalysis> sumList = this.querySmallSellSum(map, null);
		Workbook wb = new XSSFWorkbook();
		Sheet sheet1 = wb.createSheet();
		Sheet sheet2 = wb.createSheet();
		wb.setSheetName(0, "小分类信息");
		wb.setSheetName(1, "小分类同比信息");
		font = wb.createFont();
		...
		if (!smallList.isEmpty()) {
			smallList.addAll(sumList);
			this.fillMerchandiseSellDataCell(map, smallList, wb, sheet2);
		}
		try {
			wb.write(out);
		} catch (Exception e) {
	 ...
	 *            金额、数量格式
	 */
	private void writeOneRowDate(Sheet sheet, Integer rowIndex, SellYearOnYear sellYearOnYear, CellStyle strStyle, CellStyle amtStyle) {
		Row row = sheet.createRow(rowIndex + 2);

		int j = 0;
		Cell cell = row.createCell(j++);
		cell.setCellStyle(strStyle);
		cell.setCellValue(sellYearOnYear.getSmallTypeName());// 明细类名称

		cell = row.createCell(j++);
		cell.setCellStyle(strStyle);
		cell.setCellValue(sellYearOnYear.getSmallTypeSku());// SKU数
	...
	public List<SellYearOnYear> listSmallTypeInfo(Map<String, Object> map, PageInfo pageInfo) {
		return smallSellYearOnYearDao.listSmallTypeInfo(map, pageInfo);
	}

	// 查询小分类销售同比总计
	@Override
	public List<SellYearOnYearAnalysis> querySmallSellSum(Map<String, Object> map, PageInfo pageInfo) {
		return smallSellYearOnYearDao.querySmallSellSum(map, pageInfo);
	}
