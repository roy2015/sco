 ...
 * @version 1.0
 */
public class SellYearOnYearServiceImpl extends ServiceImpl implements SellYearOnYearService {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private SellYearOnYearDao sellYearOnYearDao;
	private Font font = null;

	public SellYearOnYearDao getSellYearOnYearDao() {
		return sellYearOnYearDao;
	}

	public void setSellYearOnYearDao(SellYearOnYearDao sellYearOnYearDao) {
		this.sellYearOnYearDao = sellYearOnYearDao;
	}

	public static SellYearOnYearService getInstance() {
		return (SellYearOnYearService) ConfigFactory.getInstance().getBean("sellYearOnYearService");
	}

	// 查询销售同比列表
	@Override
	public List<SellYearOnYear> listSellYearOnYear(Map<String, Object> map, PageInfo pageInfo) {
		return sellYearOnYearDao.listSellYearOnYear(map, pageInfo);
	}

	// 查询单品信息
	@Override
	public List<SellYearOnYear> queryProductInfo(Map<String, Object> map, PageInfo pageInfo) {
		return sellYearOnYearDao.queryProductInfo(map, pageInfo);
	}

	// 单品销售同比
	@Override
	public List<SellYearOnYearAnalysis> queryProductSellYearOnYear(Map<String, Object> map, PageInfo pageInfo) {
		return sellYearOnYearDao.queryProductSellYearOnYear(map, pageInfo);
	}
	...
	public String saveProductSellToHtml(String fileName, Map<String, Object> paraMap) throws Exception {
		String tarPath;
		List<SellYearOnYear> sellList = this.queryProductInfo(paraMap, null);
		List<SellYearOnYearAnalysis> merchandiseList = this.queryProductSellYearOnYear(paraMap, null);
		List<SellYearOnYearAnalysis> sumList = this.queryProductSellSum(paraMap, null);
		if (!merchandiseList.isEmpty()) {
			merchandiseList.addAll(sumList);
		}
		// paraMap.clear();
		...
		try {
			// 默认查看1年同比数据，用模版一
			String fileDirName = "sellAnalysis/sellYearOnYearAnalysis/productSellYearOnYearReport.ftl";
			tarPath = FreeMarkerUtil.generateHtml(fileDirName,
					"merchandiseSellYearOnYear".concat("/").concat(fileName), paraMap);
		} catch (Exception e) {
			return "转换报表模板时异常!";
		}
		...
		if (file.exists()) {
			try {
				Reports myReport = new Reports(BusinessConstants.myReportType.SCA.toString(), fileName,
						tarPath.replace(ConfigPath.getUploadFilePath(), ""));
				ReportsServiceImpl.getInstance().insertReports(myReport);
			} catch (Exception e) {
				file.delete();
				LoggerUtil.logger.error("SellYearOnYearServiceImpl.saveProductSellToHtml删除文件["+ file.getPath() +"]");
	...
	public void exportMerchandiseSellToExcel(Map<String, Object> map, ServletOutputStream out) throws Exception {
		List<SellYearOnYear> list = this.queryProductInfo(map, null);
		List<SellYearOnYearAnalysis> merchandiseList = this.queryProductSellYearOnYear(map, null);
		List<SellYearOnYearAnalysis> sumList = this.queryProductSellSum(map, null);
		Workbook wb = new XSSFWorkbook();
		Sheet sheet1 = wb.createSheet();
		Sheet sheet2 = wb.createSheet();
		wb.setSheetName(0, "商品信息");
		wb.setSheetName(1, "商品同比信息");
		font = wb.createFont();
	 ...
	 *            单据明细list
	 */
	private void fillDataCell(List<SellYearOnYear> list, Workbook wb, Sheet sheet) {
	 ...
	 *            金额、数量格式
	 */
	private void writeOneRowDate(Sheet sheet, Integer rowIndex, SellYearOnYear sellYearOnYear, CellStyle strStyle) {
		Row row = sheet.createRow(rowIndex + 2);

		int j = 0;
		Cell cell = row.createCell(j++);
		cell.setCellStyle(strStyle);
		cell.setCellValue(sellYearOnYear.getMerchandiseCode());// 商品编号

		cell = row.createCell(j++);
		cell.setCellStyle(strStyle);
		cell.setCellValue(sellYearOnYear.getMerchandiseName());// 商品名称

		cell = row.createCell(j++);
		cell.setCellStyle(strStyle);
		cell.setCellValue(sellYearOnYear.getDxRoleName());// 定性角色

		cell = row.createCell(j++);
		cell.setCellStyle(strStyle);
		cell.setCellValue(sellYearOnYear.getDlRoleName());// 定量角色

		cell = row.createCell(j++);
		cell.setCellStyle(strStyle);
		cell.setCellValue(sellYearOnYear.getCentreTypeName());// 中分类

		cell = row.createCell(j++);
		cell.setCellStyle(strStyle);
		cell.setCellValue(sellYearOnYear.getSmallTypeName());// 小分类

		cell = row.createCell(j++);
		cell.setCellStyle(strStyle);
		cell.setCellValue(sellYearOnYear.getDetailTypeName());// 明细类

		cell = row.createCell(j++);
		cell.setCellStyle(strStyle);
		cell.setCellValue(sellYearOnYear.getFineTypeName());// 细分类

		cell = row.createCell(j++);
		cell.setCellStyle(strStyle);
		cell.setCellValue(sellYearOnYear.getSupplierCode());// 供应商编号

		cell = row.createCell(j++);
		cell.setCellStyle(strStyle);
		cell.setCellValue(sellYearOnYear.getSupplierName());// 供应商名称
	 ...
	 *            单据明细list
	 */
	private void fillMerchandiseSellDataCell(Map<String, Object> map, List<SellYearOnYearAnalysis> list, Workbook wb,
			Sheet sheet) {
		CellStyle strStyle = wb.createCellStyle();// 字符串样式
		strStyle.setFont(font);
		ExcelUtils.setFrame(strStyle, true);
	...
	private void writeMerchandiseSellOneRowData(Map<String, Object> map, Sheet sheet, Integer rowIndex,
			SellYearOnYearAnalysis merchandiseSell, CellStyle strStyle, CellStyle dateStyle, CellStyle amtStyle) {
		...
		if (defaultYear == 1) {
			for (int j = 0; j <= 24 + (defaultYear - 1) * 12; j++) {
				...
				if (j == 0) {
					...
					cell.setCellValue(DecimalFormatUtils.bigDecimalToPercent(merchandiseSell
							.getLastSellQuantityChange()));// 去年销售量同比
				} else if (j == 5) {
					...
					cell.setCellValue(DecimalFormatUtils.bigDecimalToPercent(merchandiseSell
							.getLastSellTotalPriceChange()));// 去年销售额同比
				} else if (j == 9) {
					...
					cell.setCellValue(DecimalFormatUtils.bigDecimalToPercent(merchandiseSell
							.getLastPsdSellProfitChange()));// 去年PSD毛利额同比
				}
			}
		} else if (defaultYear == 2) {
			for (int j = 0; j <= 24 + (defaultYear - 1) * 12; j++) {
				...
				if (j == 0) {
					...
					cell.setCellValue(DecimalFormatUtils.bigDecimalToPercent(merchandiseSell
							.getBeforeSellQuantityChange()));// 前年销售量同比
				} else if (j == 7) {
					cell.setCellValue(DecimalFormatUtils.bigDecimalToString(merchandiseSell.getSellTotalPrice()));// 销售额
				} else if (j == 8) {
					cell.setCellValue(DecimalFormatUtils.bigDecimalToPercent(merchandiseSell.getSellTotalPriceChange()));// 销售额同比
					...
					cell.setCellValue(DecimalFormatUtils.bigDecimalToPercent(merchandiseSell
							.getBeforeSellTotalPriceChange()));// 前年销售额同比
				} else if (j == 13) {
					cell.setCellValue(DecimalFormatUtils.bigDecimalToString(merchandiseSell.getSellProfit()));// 毛利额
				} else if (j == 14) {
					cell.setCellValue(DecimalFormatUtils.bigDecimalToPercent(merchandiseSell.getSellProfitChange()));// 毛利额同比
					...
					cell.setCellValue(DecimalFormatUtils.bigDecimalToPercent(merchandiseSell
							.getBeforeSellProfitChange()));// 前年毛利额同比
				} else if (j == 19) {
					cell.setCellValue(DecimalFormatUtils.bigDecimalToString(merchandiseSell.getPsdSellQuantity()));// PSD销售量
				} else if (j == 20) {
					cell.setCellValue(DecimalFormatUtils.bigDecimalToPercent(merchandiseSell.getPsdSellQuantityChange()));// PSD销售量同比
					...
					cell.setCellValue(DecimalFormatUtils.bigDecimalToPercent(merchandiseSell
							.getBeforePsdSellQuantityChange()));// 前年PSD销售量同比
				} else if (j == 25) {
					cell.setCellValue(DecimalFormatUtils.bigDecimalToString(merchandiseSell.getPsdSellTotalPrice()));// PSD销售额
				} else if (j == 26) {
					cell.setCellValue(DecimalFormatUtils.bigDecimalToPercent(merchandiseSell
					...
					cell.setCellValue(DecimalFormatUtils.bigDecimalToPercent(merchandiseSell
							.getLastPsdSellTotalPriceChange()));// 去年PSD销售额同比
				} else if (j == 29) {
					cell.setCellValue(DecimalFormatUtils.bigDecimalToString(merchandiseSell
							.getBeforePsdSellTotalPrice()));// 前年PSD销售额
				} else if (j == 30) {
					cell.setCellValue(DecimalFormatUtils.bigDecimalToPercent(merchandiseSell
							.getBeforePsdSellTotalPriceChange()));// 前年PSD销售额同比
				} else if (j == 31) {
					cell.setCellValue(DecimalFormatUtils.bigDecimalToString(merchandiseSell.getPsdSellProfit()));// PSD毛利额
				} else if (j == 32) {
					cell.setCellValue(DecimalFormatUtils.bigDecimalToPercent(merchandiseSell.getPsdSellProfitChange()));// PSD毛利额同比
					...
					cell.setCellValue(DecimalFormatUtils.bigDecimalToPercent(merchandiseSell
							.getBeforePsdSellProfitChange()));// 前年PSD毛利额同比
				}
			}
		} else if (defaultYear == 3) {
			for (int j = 0; j <= 24 + (defaultYear - 1) * 12; j++) {
				...
				if (j == 0) {
					...
					cell.setCellValue(DecimalFormatUtils.bigDecimalToPercent(merchandiseSell
							.getFourthSellQuantityChange()));// 大前年销售量同比
				} else if (j == 9) {
					cell.setCellValue(DecimalFormatUtils.bigDecimalToString(merchandiseSell.getSellTotalPrice()));// 销售额
				} else if (j == 10) {
					cell.setCellValue(DecimalFormatUtils.bigDecimalToPercent(merchandiseSell.getSellTotalPriceChange()));// 销售额同比
					...
					cell.setCellValue(DecimalFormatUtils.bigDecimalToPercent(merchandiseSell
							.getFourthSellProfitChange()));// 大前年毛利额同比
				} else if (j == 25) {
					cell.setCellValue(DecimalFormatUtils.bigDecimalToString(merchandiseSell.getPsdSellQuantity()));// PSD销售量
				} else if (j == 26) {
					cell.setCellValue(DecimalFormatUtils.bigDecimalToPercent(merchandiseSell.getPsdSellQuantityChange()));// PSD销售量同比
					...
					cell.setCellValue(DecimalFormatUtils.bigDecimalToPercent(merchandiseSell
							.getFourthPsdSellQuantityChange()));// 大前年PSD销售量同比
				} else if (j == 33) {
					cell.setCellValue(DecimalFormatUtils.bigDecimalToString(merchandiseSell.getPsdSellTotalPrice()));// PSD销售额
				} else if (j == 34) {
					cell.setCellValue(DecimalFormatUtils.bigDecimalToPercent(merchandiseSell
					...
					cell.setCellValue(DecimalFormatUtils.bigDecimalToString(merchandiseSell
							.getFourthPsdSellTotalPrice()));// 前年PSD销售额
				} else if (j == 40) {
					cell.setCellValue(DecimalFormatUtils.bigDecimalToPercent(merchandiseSell
							.getFourthPsdSellTotalPriceChange()));// 前年PSD销售额同比
				} else if (j == 41) {
					cell.setCellValue(DecimalFormatUtils.bigDecimalToString(merchandiseSell.getPsdSellProfit()));// PSD毛利额
				} else if (j == 42) {
					cell.setCellValue(DecimalFormatUtils.bigDecimalToPercent(merchandiseSell.getPsdSellProfitChange()));// PSD毛利额同比
					...
					cell.setCellValue(DecimalFormatUtils.bigDecimalToPercent(merchandiseSell
							.getFourthPsdSellProfitChange()));// 大前年PSD毛利额同比
				}
			}
		}
	}
	...
	public void syncMerchandiseYoyAnalysis(Map<String, Object> map) throws Exception {
		sellYearOnYearDao.syncMerchandiseYoyAnalysis(map);
	}

	// 单品销售同比总计
	@Override
	public List<SellYearOnYearAnalysis> queryProductSellSum(Map<String, Object> map, PageInfo pageInfo) {
		return sellYearOnYearDao.queryProductSellSum(map, pageInfo);
	}
