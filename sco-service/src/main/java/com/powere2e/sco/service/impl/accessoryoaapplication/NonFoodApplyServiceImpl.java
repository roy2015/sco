 ...
 * @since 2015年4月20日
 */
public class NonFoodApplyServiceImpl extends ServiceImpl implements NonFoodApplyService {
	/**
	 * 
	 */
	...
	private NonFoodApplyDao nonFoodApplyDao;

	public static NonFoodApplyService getInstance() {
		return (NonFoodApplyService) ConfigFactory.getInstance().getBean("nonFoodApplyService");
	}
	...
	// 获得报价单DAO实例
	public NonFoodApplyDao getNonFoodApplyDao() {
		return nonFoodApplyDao;
	}

	// 设置报价单DAO实例
	public void setNonFoodApplyDao(NonFoodApplyDao nonFoodApplyDao) {
		this.nonFoodApplyDao = nonFoodApplyDao;
	}
	...
	public List<NonFoodApply> listNonFoodApplyIntentionApplication(Map<String, Object> map, PageInfo pageInfo) {
		return nonFoodApplyDao.listNonFoodApplyIntentionApplication(map, pageInfo);
	}

	// 根据报价单编号获得采购委员会竞价单OA申请意向品
	@Override
	public NonFoodApply loadNonFoodApplyIntentionApplication(String quotedCode) {
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("quotedCode", quotedCode);
		return nonFoodApplyDao.loadNonFoodApplyIntentionApplication(map);
	}
	...
	@Override
	public Boolean nonFoodInsertUpdateDeleteIsOk(String quotedCodes, String applicationCode, String applicationType) {
		Boolean isOk = true;
		try {
	...
	public void insertWlInfo(WlInfo[] dataArray) {
		for (WlInfo wlInfo : dataArray) {
			...
			nonFoodApplyDao.updateIntentionSupplierAccessory(map);
		}
	}
}