...
import com.powere2e.sco.interfaces.service.sharefunctionanalysis.sellanalysis.selllinkrelative.MarketLinkRelativeService;
 ...
 * @version 1.0
 */
public class MarketLinkRelativeServiceImpl extends ServiceImpl implements MarketLinkRelativeService {

	/**
	 * 
	 */
	private static final long serialVersionUID = -2039767275696916937L;
	private MarketLinkRelativeDao marketLinkRelativeDao;
	private Font font = null;

	public MarketLinkRelativeDao getMarketLinkRelativeDao() {
		return marketLinkRelativeDao;
	}

	public void setMarketLinkRelativeDao(MarketLinkRelativeDao marketLinkRelativeDao) {
		this.marketLinkRelativeDao = marketLinkRelativeDao;
	}

	public static MarketLinkRelativeService getInstance() {
		return (MarketLinkRelativeService) ConfigFactory.getInstance().getBean("marketLinkRelativeService");
	}

	// 查询整体销售环比
	public List<SellLinkRelativeAnalysis> queryMarketSellLinkRelative(Map<String, Object> map, PageInfo pageInfo) {
		return marketLinkRelativeDao.queryMarketSellLinkRelative(map, pageInfo);
	}
	...
	public String saveMarketSellToHtml(String fileName, Map<String, Object> paraMap) throws Exception {
		String tarPath = null;
		List<SellLinkRelativeAnalysis> merchandiseList = this.queryMarketSellLinkRelative(paraMap, null);

		paraMap.put("merchandiseList", merchandiseList);

		try {
			String fileDirName = "sellAnalysis/sellLinkRelative/marketSellLinkRelativeReport.ftl";
			tarPath = FreeMarkerUtil.generateHtml(fileDirName, "marketSellLinkRelative".concat("/").concat(fileName), paraMap);
		} catch (Exception e) {
			return "转换报表模板时异常!";
		}
		...
		if (file.exists()) {
			try {
				Reports myReport = new Reports(BusinessConstants.myReportType.GSS.toString(), fileName, tarPath.replace(ConfigPath.getUploadFilePath(), ""));
				ReportsServiceImpl.getInstance().insertReports(myReport);
			} catch (Exception e) {
				file.delete();
				LoggerUtil.logger.error("MarketLinkRelativeServiceImpl.saveMarketSellToHtml删除文件["+ file.getPath() +"]");
	...
	public void exportMarketSellToExcel(Map<String, Object> map, ServletOutputStream out) throws Exception {
		List<SellLinkRelativeAnalysis> merchandiseList = this.queryMarketSellLinkRelative(map, null);
		Workbook wb = new XSSFWorkbook();
		Sheet sheet = wb.createSheet();
		wb.setSheetName(0, "整体环比信息");
		font = wb.createFont();
		...
		if (!merchandiseList.isEmpty()) {
			this.fillMerchandiseSellDataCell(map, merchandiseList, wb, sheet);
		}
		try {
			wb.write(out);
		} catch (Exception e) {
