 ...
 * 
 * @author Joyce.li
 * @since 2015年6月8日 下午7:11:45
 * @version 1.0
 */
public class DetailSellYearOnYearServiceImpl extends ServiceImpl implements DetailSellYearOnYearService {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private DetailSellYearOnYearDao detailSellYearOnYearDao;
	private Font font = null;

	public DetailSellYearOnYearDao getDetailSellYearOnYearDao() {
		return detailSellYearOnYearDao;
	}

	public void setDetailSellYearOnYearDao(DetailSellYearOnYearDao detailSellYearOnYearDao) {
		this.detailSellYearOnYearDao = detailSellYearOnYearDao;
	}

	public static DetailSellYearOnYearService getInstance() {
		return (DetailSellYearOnYearService) ConfigFactory.getInstance().getBean("detailSellYearOnYearService");
	}

	// 查询明细类信息
	@Override
	public List<SellYearOnYear> listDetailTypeInfo(Map<String, Object> map, PageInfo pageInfo) {
		return detailSellYearOnYearDao.listDetailTypeInfo(map, pageInfo);
	}

	// 查询明细类同比结果
	@Override
	public List<SellYearOnYearAnalysis> queryDetailSellYearOnYear(Map<String, Object> map, PageInfo pageInfo) {
		return detailSellYearOnYearDao.queryDetailSellYearOnYear(map, pageInfo);
	}
	...
	public String saveDetailSellToHtml(String fileName, Map<String, Object> paraMap) throws Exception {
		String tarPath;
		List<SellYearOnYear> detailTypeList = this.listDetailTypeInfo(paraMap, null);
		List<SellYearOnYearAnalysis> detailSellList = this.queryDetailSellYearOnYear(paraMap, null);
		List<SellYearOnYearAnalysis> detailSellSumList = this.queryDetailSellSum(paraMap, null);
		if (!detailSellList.isEmpty()) {
			detailSellList.addAll(detailSellSumList);
		}
		// paraMap.clear();
		paraMap.put("detailTypeList", detailTypeList);
		paraMap.put("detailSellList", detailSellList);
		// String seeYear = (String) paraMap.get("seeYear");

		try {
			// 默认查看1年同比数据，用模版一
			String fileDirName = "sellAnalysis/sellYearOnYearAnalysis/detailSellYearOnYearReport.ftl";
			tarPath = FreeMarkerUtil.generateHtml(fileDirName, "detailSellYearOnYear".concat("/").concat(fileName), paraMap);
		} catch (Exception e) {
			return "转换报表模板时异常!";
		}
		...
		if (file.exists()) {
			try {
				Reports myReport = new Reports(BusinessConstants.myReportType.DCA.toString(), fileName, tarPath.replace(ConfigPath.getUploadFilePath(), ""));
				ReportsServiceImpl.getInstance().insertReports(myReport);
			} catch (Exception e) {
				file.delete();
				LoggerUtil.logger.error("> DetailSellYearOnYearServiceImpl.saveDetailSellToHtml删除文件["+ file.getPath() +"]");
	...
	public void exportDetailSellToExcel(Map<String, Object> map, ServletOutputStream out) throws Exception {
		List<SellYearOnYear> list = this.listDetailTypeInfo(map, null);
		List<SellYearOnYearAnalysis> detailList = this.queryDetailSellYearOnYear(map, null);
		List<SellYearOnYearAnalysis> sumList = this.queryDetailSellSum(map, null);
		Workbook wb = new XSSFWorkbook();
		Sheet sheet1 = wb.createSheet();
		Sheet sheet2 = wb.createSheet();
		wb.setSheetName(0, "明细类信息");
		wb.setSheetName(1, "明细类同比信息");
		font = wb.createFont();
		...
		if (!detailList.isEmpty()) {
			detailList.addAll(sumList);
			this.fillMerchandiseSellDataCell(map, detailList, wb, sheet2);
		}
		try {
			wb.write(out);
		} catch (Exception e) {
	...
	private void writeOneRowDate(Sheet sheet, Integer rowIndex, SellYearOnYear sellYearOnYear, CellStyle strStyle, CellStyle amtStyle) {
		...
		cell.setCellValue(sellYearOnYear.getDetailTypeName());// 明细类名称

		cell = row.createCell(j++);
		cell.setCellStyle(strStyle);
		cell.setCellValue(sellYearOnYear.getDetailTypeSku());// SKU数
	...
	public List<SellYearOnYearAnalysis> queryDetailSellSum(Map<String, Object> map, PageInfo pageInfo) {
		return detailSellYearOnYearDao.queryDetailSellSum(map, pageInfo);
	}
